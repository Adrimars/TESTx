...
- **Registration:** email/password, Google, or Apple (iOS), followed by the demographic profile-onboarding step (existing `evaluatorProfileSchema` fields). Requires an **18+ age gate** (self-declared, checked against the `age` field already collected) — sidesteps the unresolved Turkish Civil Code question of whether a minor can validly consent, rather than trying to answer it (see `kvkk-compliance-research.md` §5). This in-app gate is independent of the store's own age-rating/content questionnaire (App Store Connect / Google Play IARC) — those should be answered honestly based on actual app content, which has no mature themes by design and should land well below an 18+ store rating (`appstore-playstore-compliance-research.md` §5).
...
- **Profile screen:** every `EvaluatorProfile` field is editable post-registration (age, gender, country, city, languages, occupation, education, AI-usage fields) via `PUT /evaluator/profile`, plus an **avatar picker** — no photo upload; the evaluator chooses one of **8–10 preset avatar images bundled with the app** (illustrated/generic, not user photos). Stored as `User.avatarId` (a small integer/key identifying which bundled preset was chosen, see §12.4), not a URL — there is no upload endpoint and no user-submitted photographic data at all.
- **Logout:** standard, clears the stored `expo-secure-store` tokens and returns to the entry screen.
- **Account deletion (required for store submission, not optional):** Apple Guideline 5.1.1(v) and Google Play's account-deletion policy both mandate genuine in-app, self-service account deletion the moment an app supports account creation — deactivation/sign-out alone does not satisfy either, and there is no MVP exception (`appstore-playstore-compliance-research.md` §3). A discoverable "Delete Account" action on the profile screen, with confirmation, calls a `DELETE /users/me` backend endpoint; Google Play additionally requires a web-reachable deletion-request path outside the app. This is a hard pre-submission blocker, not a nice-to-have.
