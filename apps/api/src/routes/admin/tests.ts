...
async function injectQualityQuestions(app: Parameters<FastifyPluginAsync>[0], testId: string) {
  const questions = await app.prisma.question.findMany({
    where: { testId },
    orderBy: { order: "asc" },
  ...
  // Skip if already injected (re-activation from PAUSED guard)
  const alreadyInjected = questions.some((q) => {
    const cfg = q.config as Record<string, unknown>;
    return cfg?.autoGenerated === true;
  });
  if (alreadyInjected) return;

  // Only count regular questions (not any manually placed special questions)
  const regular = questions.filter((q) => !q.isAttentionCheck && !q.isTrapDuplicate);
  if (regular.length === 0) return;

  // Split regular questions into groups of 3
  const groups: (typeof regular)[] = [];
  for (let i = 0; i < regular.length; i += 3) {
    groups.push(regular.slice(i, i + 3));
  }

  type Injection = {
    afterQuestionId: string;
    kind: "attention" | "trap";
    source: (typeof regular)[0] | undefined;
  };

  // Alternate: even group index → attention, odd → trap (fallback to attention if no select-type)
  const injections: Injection[] = groups.map((group, gi) => {
    const last = group[group.length - 1]!;
    const selectSource = group.find((q) => q.type === "SINGLE_SELECT" || q.type === "MULTI_SELECT");
    const kind: "attention" | "trap" = gi % 2 !== 0 && selectSource ? "trap" : "attention";
    return {
      afterQuestionId: last.id,
      kind,
      source: kind === "trap" ? selectSource : group.find((q) => q.options.length >= 2),
    };
  });

  // Build map: questionId → injections to insert after it
  const insertAfter = new Map<string, Injection[]>();
  for (const inj of injections) {
    const list = insertAfter.get(inj.afterQuestionId) ?? [];
    list.push(inj);
    insertAfter.set(inj.afterQuestionId, list);
  }

  // Assign final order values interleaving existing + injected
  let nextOrder = 1;
  const orderUpdates: { id: string; order: number }[] = [];
  const creates: { order: number; injection: Injection }[] = [];

  for (const q of questions) {
    orderUpdates.push({ id: q.id, order: nextOrder++ });
    for (const inj of insertAfter.get(q.id) ?? []) {
      creates.push({ order: nextOrder++, injection: inj });
    }
  }

  function shuffleOptions(opts: (typeof regular)[0]["options"]) {
    const arr = [...opts];
    for (let i = arr.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [arr[i], arr[j]] = [arr[j]!, arr[i]!];
    }
    // Guarantee a different order — rotate by 1 if shuffle left it unchanged
    if (arr.length > 1 && arr.every((o, i) => o.id === opts[i]?.id)) {
      arr.push(arr.shift()!);
    }
    return arr;
  }
  ...
  await app.prisma.$transaction(async (tx) => {
    // Use large negative intermediates to avoid (testId, order) unique conflicts
    for (const u of orderUpdates) {
      await tx.question.update({ where: { id: u.id }, data: { order: -(u.order * 1000) } });
    }
    for (const u of orderUpdates) {
      await tx.question.update({ where: { id: u.id }, data: { order: u.order } });
    }

    for (const { order, injection } of creates) {
      if (injection.kind === "attention") {
        const src = injection.source;
        if (src && src.options.length >= 2) {
          const correct = src.options[0]!;
          await tx.question.create({
            data: {
              testId,
              type: "SINGLE_SELECT",
              prompt: `Attention check: select "${correct.label ?? "the first option"}".`,
              mediaType: src.mediaType,
              order,
              isAttentionCheck: true,
              config: {
                autoGenerated: true,
                correctOptionOrder: correct.order,
                correctOptionLabel: correct.label ?? null,
              } as Prisma.InputJsonValue,
              options: { create: src.options.map((o) => ({ label: o.label, mediaId: o.mediaId, order: o.order })) },
            },
          });
        } else {
          await tx.question.create({
            data: {
              ...
              order,
              isAttentionCheck: true,
              config: { autoGenerated: true, correctOptionLabel: "I am paying attention" } as Prisma.InputJsonValue,
              options: { create: [{ label: "I am paying attention", order: 1 }, { label: "Skip this option", order: 2 }] },
            },
          });
        }
      } else {
        const src = injection.source!;
        const shuffled = shuffleOptions(src.options);
        await tx.question.create({
          data: {
            testId,
            type: src.type as "SINGLE_SELECT" | "MULTI_SELECT",
            prompt: src.prompt,
            mediaType: src.mediaType,
            order,
            isTrapDuplicate: true,
            trapSourceId: src.id,
            config: { autoGenerated: true } as Prisma.InputJsonValue,
            options: { create: shuffled.map((o, i) => ({ label: o.label, mediaId: o.mediaId, order: i + 1 })) },
          },
        });
      }
    }
...
export const adminTestsRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.put<{ Params: { id: string } }>("/tests/:id/status", adminAuth, async (request, reply) => {
    ...
    if (status === "ACTIVE") {
      await injectQualityQuestions(app, existing.id);
    }
  ...
  app.post<{ Params: { id: string } }>("/tests/:id/questions", adminAuth, async (request, reply) => {
    ...
    body.isAttentionCheck = false;
    body.isTrapDuplicate = false;
    body.trapSourceId = null;
