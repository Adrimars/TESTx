...
import { MotionPress } from "@/components/motion-surface";
...
const FLAG_REASON_LABELS: Record<string, string> = {
  SPEED_TOO_FAST:
    "You finished the test too quickly — the whole test took less than the minimum time it requires.",
  TIMING_UNVERIFIED:
    "The session was left open for too long before answers were submitted. Response times could not be verified.",
  ATTENTION_CHECK_FAILED:
    "An attention check question was answered incorrectly.",
  CONSISTENCY_FAILED:
    "Your answers were inconsistent — a repeated question received a different answer than the original.",
};
  ...
  async function handleSubmit() {
    ...
    try {
      const res = await apiFetch<SubmitResult>(
        `/evaluator/tests/${test.id}/submit`,
        {
          method: "POST",
          body: JSON.stringify({
        ...
        <div className="mx-auto max-w-2xl space-y-6 pb-24 sm:pb-0">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Check your answers before submitting. Use “Change” to go back to a
              question.
            </p>
          </div>
              ...
              .map((question, index) => {
                ...
                if (answer) {
                  if (
                    ...
                    // A ranking answer's positions are part of the answer.
                    answerText =
                  ...
                  >
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        const realIndex = test.questions.findIndex(
                          (q) => q.id === question.id,
                        );
                        router.push(
                          `/tests/${params.id}/question/${realIndex + 1}`,
                        );
                      }}
                    >
                      Change
          ...
          (result.isFlagged ? (
            <div className="space-y-5 p-6 text-center">
              <div className="flex flex-col items-center gap-3">
                ...
                <p className="text-sm text-muted-foreground">
                  Your response was flagged for quality issues. No points were
                      ...
                      <span>
                        ...
                        {reason === "SPEED_TOO_FAST" &&
                          ...
                          requiredSeconds > 0 && (
                            <>
                              {" "}
                              This test needs at least{" "}
                              {formatRequiredTime(requiredSeconds)} in total.
                            </>
                          )}
                      </span>
                    </li>
